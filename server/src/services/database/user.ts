...
import {JSON_FILE_FORMAT, USER_DATA_DIR} from '../../shared/config'
import {AMQSongListError} from '../../shared/exceptions'
import {readFile, writeFile} from './init'
import {UserSongIds} from '../../../../shared-modules/interfaces/game'
...
class UserService {
    ...
    getUserSongIds(users: Array<string>): UserSongIds {
        let userSongIds = {}
        for (let user of users) {
            this._validateUser(user)
            userSongIds[user] = this._userLists[user].list
        }
        return userSongIds
    }
